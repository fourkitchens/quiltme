...
/**
 * Helper function to recursively create a directory tree.
 *
 * @param string $path
 ...
 *
 *   Callers should *always* do their own
 *   error handling after calling drush_mkdir.
 *   If $required is FALSE, then a different location
 *   should be selected,and a final error message should be displayed
 *   if no usable locations can be found.
 *
    ...
    if (drush_mkdir(dirname($path))) {
      ...
      elseif (is_dir($path) && is_writable($path)) {
        ...
        if (is_writable(dirname($path))) {
          ...
          return dt('Unable to create !newdir in !dir. Please check directory permissions.', [
